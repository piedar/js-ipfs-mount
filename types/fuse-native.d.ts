...
declare module "fuse-native" {
  ...
  class Fuse {
    static isConfigured(cb: (is: boolean) => void): void;
    static configure(cb: () => void): void;
    static unconfigure(cb: () => void): void;

    constructor(mnt: string, handlers: Fuse.Handlers, options?: Fuse.MountOptions)

    mount(cb: (err?: any) => void): void;
    unmount(cb: (err?: any) => void): void;
  }
  export = Fuse
  ...
  namespace Fuse {
    ...
    type MountOptions = {
      /** Adds a name/icon to the mount volume on OSX. */
      displayFolder?: string,
      /** Enables detailed tracing of operations. */
      debug?: boolean,
      /** Attempts to unmount the mountpoint before remounting. */
      force?: boolean,
      /** Attempts to create the mountpoint before mounting. */
      mkdir?: boolean,
    }
    ...
    type Handlers = {
      ...
      /**
       * Called when a path is being opened. Similar to open(), but for directories.
       * @param path
       * @param flags in a number containing the permissions being requested.
       * @param cb accepts a file descriptor after the return code.
       */
